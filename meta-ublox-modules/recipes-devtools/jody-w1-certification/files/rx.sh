...
then
    ...
    band=a
    hint="(20,40,80):"
    echo -n "   enter channel bandwidth ${hint} "
    read wlanChannelBW
fi

viewStep $step "set RX channel: $wlanChannel"
...
viewStep $step "set calibration data: $wlanDataRate"
...
$wl phy_activecal &>/dev/null
if [ $? -ne 0 ]
then
    myExit $step  "wl phy_activecal"
fi
...
# 6 ================= set core and start rx ===========================
hint="(1=core0, 2=core1):"
echo -n "   enter core $hint "
read core

viewStep $step "reset counters; start receive on core $core"

# select core
$wl rxchain $core &>/dev/null
if [ $? -ne 0 ]
then
    myExit $step  "wl rxchain $core"
fi

# reset rx counter:
$wl reset_cnts &>/dev/null
if [ $? -ne 0 ]
then
    myExit $step  "wl reset_cnts"
fi

viewSuccess

echo
read -p "*** Start TX from generator. Press Enter to see RX counters: " qwe
